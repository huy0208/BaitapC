...
void sortArray(typeArray *value)
{
    ...
    for(int i = 0; i < (value->size) - 1; i++){
        for(int j = i + 1; j < value ->size; j++){
            if(value->firstAdd[i] > value->firstAdd[j])// so sanh phan tu hien tai va tiep theo
            {
...
void coutArray2(typeArray *value)
{
    ...
    for(int i=0; i < (value->size);i++)
    {
       for(int j=i+1; j < (value->size) ;j++)
       {
            if(value->firstAdd[i] == value->firstAdd[j] & value->firstAdd[i] != 0)
            {
                dem++;
                value->firstAdd[j]=0;
            }
